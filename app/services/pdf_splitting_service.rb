...
class PdfSplittingService
  ...
  PO_PATTERNS = [
    ...
    /PO\s*#\s*([A-Z]?\d+)/i,                       # e.g., "PO # 45580095"
    /PO\s+No:\s*([A-Z]?\d+)/i,                     # e.g., "PO No: 1114236"
    /Purchase Order No#?\s*([A-Z]?\d+)/i           # e.g., "Purchase Order No# 570394" or "Purchase Order No 12345"
  ].freeze
  ...
  def find_po_locations
    ...
    @reader.pages.each_with_index do |page, page_index|
      ...
      if po_number && !seen_pos.include?(po_number)
        po_locations << {
          po_number: po_number,
          page: page_number,
          end_page: nil
